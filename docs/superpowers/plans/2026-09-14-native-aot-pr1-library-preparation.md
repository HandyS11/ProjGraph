...
2. **Reflection-based JSON is disabled.** `ProjGraph.Mcp` sets `JsonSerializerIsReflectionEnabledByDefault=false`. Without it, the JIT build silently falls back to reflection, and the snapshot can't detect a type missing from `McpJsonContext` or options that were never passed. The switch only reaches the real executable: with it set, a missing registration (verified by removing the options from `WithTools`) fails only the tests that launch the server over stdio. The in-process MCP tests still run with reflection enabled in the test host, so they can't catch it.
...
- A `.sln` that lists the same project path twice (under different project GUIDs) now throws `ParsingException`, so `visualize`/`stats` fail for the whole solution. Microsoft.Build returned both entries and `BuildGraphUseCase` deduplicated them downstream. `SlnParserTests.GetProjectPaths_SamePathListedTwice_ShouldThrowParsingException` pins the new behavior.
- These `.sln` inputs made Microsoft.Build throw and now parse: two projects with the same display name, a project entry with an empty path, a solution folder with the same name as a project, a `NestedProjects` entry that points to a missing parent, and a malformed `ProjectConfigurationPlatforms` line.
- **BCL binding.** The embedded reference assemblies bind the same public BCL surface as the runtime set (`System.Private.CoreLib`, `System.Runtime`, `System.Collections`, `System.ComponentModel.Annotations`), except 19 top-level public types that have no reference contract (implementation-only types such as ``System.Collections.Generic.TreeSet`1``, `System.CultureAwareComparer`, `System.Reflection.Emit.PEFileKinds`). In the other direction, 118 types the runtime set couldn't bind now bind (for example ``System.Collections.Concurrent.ConcurrentDictionary`2``, ``System.Buffers.IBufferWriter`1``, `System.Threading.Barrier`, and the COM interop types). `TypeFilter` now treats them as System types, so diagrams that showed them as nodes no longer do.
