...
3. **The release smoke installs the tools with `dotnet tool install`** from the pointer package plus the native package, instead of extracting executables from the `.nupkg`.
   ...
   - The install uses a fresh package folder and a NuGet config that lists the local packages first.
   - NuGet.org is also listed, because the first PR run showed `dotnet tool install` on macOS arm64 always downloading `microsoft.netcore.app.host.osx-x64` for RID-specific packages. The SDK reads `tools/any` as a pre-net6 framework, even though a native tool only gets a symlink. No ProjGraph package with the CI version (`0.0.0-ci.<run>`) exists on NuGet.org, so the tools still resolve from the local packages.
   - On Windows the SDK writes `.cmd` launchers into the tool path, not links. There the smoke suite runs `ProjGraph.Cli.exe` and `ProjGraph.Mcp.exe` from the tool store, which are the shipped executables. This also makes `NativeBuildGuardTests` meaningful on Windows.
...
jobs:
  pack-native:
    ...
    steps:
      - uses: actions/checkout@v7
        with:
          # The release tag when called from publish.yml; the pull request's merge commit otherwise.
